...
if (isset($_SESSION['member_id'])) {
    ...
    $stmt = $conn->prepare("SELECT product_id FROM favorite WHERE member_id = ? AND table_name = 'pr2'");
...
if (isset($_SESSION['member_id'])) {
    $member_id = $_SESSION['member_id'];
    $stmt = $conn->prepare("SELECT product_id FROM shopping_cart WHERE member_id = ? AND table_name = 'pr2'");
    $stmt->bind_param("i", $member_id);
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
  ...
  <div id="homeCarousel" class="carousel slide carousel-fade" data-bs-ride="carousel" data-bs-interval="4000">
    ...
    <div class="carousel-inner text-center">
      <div class="carousel-item active">
        <img src="images02/00003.png" class="d-block mx-auto w-75" alt="圖片1">
      </div>
      <div class="carousel-item">
        <img src="images02/00006.png" class="d-block mx-auto w-75" alt="圖片2">
      </div>
            <div class="carousel-item">
        <img src="images02/00007.png" class="d-block mx-auto w-75" alt="圖片2">
      </div>
    </div>
...
<!-- 篩選器 -->
 <div id="product-wrapper" style="position: relative;">
    ...
    <select name="filter" id="filter" onchange="this.form.submit()">
        ...
        <option value="type:4" <?= (isset($_GET['filter']) && $_GET['filter'] == 'type:4') ? 'selected' : '' ?>>項鍊</option>
        <option value="type:5" <?= (isset($_GET['filter']) && $_GET['filter'] == 'type:5') ? 'selected' : '' ?>>手鍊</option>
        <option value="type:6" <?= (isset($_GET['filter']) && $_GET['filter'] == 'type:6') ? 'selected' : '' ?>>戒指</option>
    </select>
</form>
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
            ...
            if (!empty($filter)) {
                if (strpos($filter, ':') !== false) {
                    ...
                    if ($field === 'type') {
                        $type = intval($value);
                        $stmt = $conn->prepare("SELECT * FROM pr2 WHERE type = ?");
                        $stmt->bind_param("i", $type);
                        $stmt->execute();
                        $result = $stmt->get_result();
                    } 
                     else {
                        $result = $conn->query("SELECT * FROM pr2");
                    }
                } else {
                    $result = $conn->query("SELECT * FROM pr2");
                }
