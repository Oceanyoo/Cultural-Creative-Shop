...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($new !== $confirm) {
        $error = "兩次輸入的新密碼不一致。";
    } elseif ($current === $db_password) {
        $update = $conn->prepare("UPDATE member SET Password = ? WHERE id = ?");
        $update->bind_param("si", $new, $member_id);
        $update->execute();
        $success = "密碼已更新！";
            ...
            <div class="card shadow">
                <div class="card-body">
                    ...
                    <form method="post">
                        ...
                        <div class="mb-3">
                            <label for="confirm_password" class="form-label">再次輸入新密碼</label>
